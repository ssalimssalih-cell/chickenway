...
function saveProduct() {
  ...
  // SYNC VERS FIREBASE ET RECHARGE
  setTimeout(async () => {
    if (typeof window.syncAllDataToFirebase === 'function') {
      await window.syncAllDataToFirebase();
      console.log("✅ Produit synchronisé vers Firebase");
    }
    
    // RECHARGER LES PRODUITS DEPUIS FIREBASE
    if (typeof window.loadProductsFromFirebase === 'function') {
      ...
      if (firebaseProducts && firebaseProducts.length > 0) {
        ...
        saveProduitsToLocal();
        console.log(`✅ ${produits.length} produits rechargés depuis Firebase`);
      }
    }
...
async function renderProductsTable() {
  ...
  // CHARGER DEPUIS FIREBASE À CHAQUE AFFICHAGE
  let produitsAfficher = produits;
  
  if (typeof window.loadProductsFromFirebase === 'function') {
    try {
      const firebaseProducts = await window.loadProductsFromFirebase();
      if (firebaseProducts && firebaseProducts.length > 0) {
        produitsAfficher = firebaseProducts;
        produits = firebaseProducts;
        saveProduitsToLocal();
        console.log(`📦 ${produits.length} produits chargés depuis Firebase pour l'affichage`);
      }
    } catch(e) {
  ...
  let filteredProduits = [...produitsAfficher];
  if (searchTerm !== '') {
  ...
  tbody.innerHTML = filteredProduits.map(p => {
    ...
    <tr>
      <td>${p.id}</td>
      <tr>${p.image ? `<img src="${p.image}" class="product-img" style="width:40px;height:40px;object-fit:cover;border-radius:6px;" onerror="this.style.display='none'">` : '📷'}</td>
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    // D'abord charger les données (Firebase en priorité)
    await chargerToutesLesDonnees();
    
    // Puis charger les démos si nécessaire
    if (typeof seedDemoData === 'function' && categories.length === 0 && produits.length === 0) {
    ...
    // Démarrer l'écoute des commandes en ligne
    if (typeof demarrerEcoutesCommandesEnLigne === 'function') {
        demarrerEcoutesCommandesEnLigne();
    }
    
    console.log("✅ Application prête !");
    console.log(`📊 Affichage: ${categories.length} catégories, ${produits.length} produits`);
});
...
async function chargerToutesLesDonnees() {
    try {
        ...
        // 1. PRIORITÉ ABSOLUE : Charger depuis Firebase d'abord
        let firebaseCategoriesLoaded = false;
        ...
        if (typeof window.loadProductsFromFirebase === 'function') {
            ...
            if (firebaseProducts && firebaseProducts.length > 0) {
                produits = firebaseProducts;
                saveProduitsToLocal();
                console.log(`✅ ${produits.length} produits chargées depuis Firebase`);
                firebaseProductsLoaded = true;
            }
        }
        
        // 2. Si Firebase n'a rien, essayer IndexedDB
        if (!firebaseCategoriesLoaded && typeof getAllCategories !== 'undefined') {
            const categoriesDB = await getAllCategories();
            if (categoriesDB.length > 0) {
                categories = categoriesDB;
                console.log(`📦 ${categories.length} catégories depuis IndexedDB`);
            }
        }
        
        if (!firebaseProductsLoaded && typeof getAllProducts !== 'undefined') {
            const produitsDB = await getAllProducts();
            if (produitsDB.length > 0) {
                produits = produitsDB;
                console.log(`📦 ${produits.length} produits depuis IndexedDB`);
            }
        }
        ...
        console.log(`📊 Final: ${categories.length} catégories, ${produits.length} produits`);
        
        if (typeof mettreAJourBadgeCommandes === 'function') mettreAJourBadgeCommandes();
        
        // Synchroniser les données locales vers Firebase si nécessaire
        if (produits.length > 0 && typeof window.syncAllDataToFirebase === 'function') {
            await window.syncAllDataToFirebase();
        }
