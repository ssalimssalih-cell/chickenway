    ...
    auth.onAuthStateChanged(function(user) {
        if (user) {
            ...
            db.collection('users').doc(user.uid).get().then(function(doc) {
                ...
                if (userData.authorized !== 'yes') { auth.signOut(); showAuthPage(); setTimeout(function(){showLoginError('Compte en attente de validation.');},300); return; }
            ...
            }).catch(function(err) { console.error('Erreur user:', err); auth.signOut(); showAuthPage(); });
        } else { currentUser = null; window.currentUserData = null; localStorage.removeItem('currentUser'); showAuthPage(); }
    });
    showLogin();
}

function toggleSidebar() { var sidebar=document.getElementById('sidebar'), overlay=document.getElementById('sidebarOverlay'); if(sidebar)sidebar.classList.toggle('open'); if(overlay)overlay.classList.toggle('active'); }
function toggleClientSidebar() { var sidebar=document.getElementById('clientSidebar'), overlay=document.getElementById('clientSidebarOverlay'); if(sidebar)sidebar.classList.toggle('open'); if(overlay)overlay.classList.toggle('active'); }
...
function showDashboard() {
    ...
    if (window.currentUserData && window.currentUserData.userData.role==='caissier') { navigateTo('pos'); } else { navigateTo('dashboard'); }
}
