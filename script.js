...
function navigateTo(page) {
    ...
    var titles = {dashboard:'Dashboard',pos:'Point de Vente (POS)',categories:'Categories',products:'Produits',clients:'Clients',fournisseurs:'Fournisseurs',ventes:'Ventes',credits:'Credits',depenses:'Depenses',statistiques:'Statistiques',options:'Options'};
    ...
    if (page === 'dashboard') {
        ...
        content.innerHTML = '<div class="content-card"><div class="card-header"><h3><i class="fas fa-cash-register"></i> Point de Vente (POS)</h3></div><div style="text-align:center;padding:60px;"><i class="fas fa-cash-register" style="font-size:4rem;color:#f39c12;"></i><p style="margin-top:20px;font-size:1.2rem;">Interface POS</p><p style="color:#64748b;">En cours de developpement</p></div></div>';
...
function clientNavigate(page) {
    ...
    var index = pages.indexOf(page);
    if (index >= 0 && items[index]) items[index].classList.add('active');
    var titles = {commander:'Commander en ligne',historique:'Historique',parametres:'Parametres'};
    document.getElementById('clientPageTitle').textContent = titles[page];
    var content = document.getElementById('clientDynamicContent');
    
    if (page === 'commander') {
        content.innerHTML = '<div class="content-card"><div class="card-header"><h3>Commander en ligne</h3></div><div style="text-align:center;padding:60px;"><i class="fas fa-utensils" style="font-size:4rem;color:#f39c12;"></i><p style="font-size:1.3rem;font-weight:600;margin-top:20px;">Menu du restaurant</p><button class="btn-add" style="margin-top:20px;padding:12px 30px;" onclick="alert(\'Fonctionnalite a venir\')">Voir le menu</button></div></div>';
    } else if (page === 'historique') {
        content.innerHTML = '<div class="content-card"><div class="card-header"><h3>Historique</h3></div><table class="data-table"><thead><tr><th>N</th><th>Date</th><th>Total</th><th>Statut</th></tr></thead><tbody id="clientOrdersTable"><tr><td colspan="4">Aucune commande</td></tr></tbody></table></div>';
        loadClientOrders();
    } else if (page === 'parametres') {
        content.innerHTML = '<div class="content-card"><div class="card-header"><h3>Parametres</h3></div><div id="profileMessage" style="display:none;padding:12px;border-radius:8px;margin-bottom:15px;"></div><p><strong>Email:</strong> ' + currentUserData.email + '</p><p><strong>Telephone:</strong> ' + (currentUserData.telephone || 'Non defini') + '</p><form onsubmit="return updateClientProfile(event)" style="margin-top:20px;"><div class="input-group" style="margin-bottom:15px;"><i class="fas fa-phone"></i><input type="tel" id="clientTelephone" value="' + (currentUserData.telephone || '') + '" placeholder="Telephone"></div><div class="input-group" style="margin-bottom:15px;"><i class="fas fa-lock"></i><input type="password" id="clientNewPassword" placeholder="Nouveau mot de passe (6+)"></div><div class="input-group" style="margin-bottom:15px;"><i class="fas fa-lock"></i><input type="password" id="clientConfirmPassword" placeholder="Confirmer mot de passe"></div><button type="submit" class="btn-add" style="width:100%;justify-content:center;padding:12px;">Enregistrer</button></form></div>';
    }
}
...
function handleLogin(event) {
    ...
    auth.signInWithEmailAndPassword(email, password).then(function(userCredential) {
        return db.collection('users').doc(userCredential.user.uid).get().then(function(doc) {
            if (!doc.exists) { throw new Error('NO_DOCUMENT'); }
            var userData = doc.data();
            if (userData.authorized !== 'yes') {
                auth.signOut(); showLoginError('Compte en attente de validation.');
                btn.disabled = false; btn.innerHTML = '<i class="fas fa-sign-in-alt"></i> Se connecter'; return;
            }
            currentUserData = { uid: doc.id, ...userData };
            localStorage.setItem('currentUser', JSON.stringify(currentUserData));
            if (userData.role === 'client') { showClientPage(); } else { showDashboard(); }
...
function showLoginError(message) {
    ...
    if (!errorEl) {
        ...
        document.getElementById('loginForm').parentNode.insertBefore(errorEl, document.getElementById('loginForm'));
    }
    errorEl.innerHTML = 'X ' + message; errorEl.style.display = 'block';
}
...
function handleRegister(event) {
    ...
    auth.createUserWithEmailAndPassword(email, password).then(function(userCredential) {
        return db.collection('users').doc(userCredential.user.uid).set({
            nom:nom,prenom:prenom,username:username,email:email,telephone:telephone,role:role,authorized:'no',
            createdAt: firebase.firestore.FieldValue.serverTimestamp()
        });
    }).then(function() {
        alert('Compte cree ! En attente de validation.'); document.getElementById('registerForm').reset(); showLogin();
    }).catch(function(e) { alert('Erreur: '+e.message); }).finally(function() { btn.disabled = false; btn.innerHTML = '<i class="fas fa-user-plus"></i> Creer mon compte'; });
    return false;
}
...
function updateSidebarUserInfo() {
    ...
    if (el && currentUserData) { el.innerHTML = '<i class="fas fa-user-circle"></i> '+currentUserData.prenom+' '+currentUserData.nom+' <small style="color:#f39c12;">('+currentUserData.role+')</small>'; }
}
function updateClientSidebarInfo() {
    var el = document.getElementById('clientSidebarInfo');
    if (el && currentUserData) { el.innerHTML = '<i class="fas fa-user-circle"></i> '+currentUserData.prenom+' '+currentUserData.nom; }
}
...
function loadUsersList() {
    db.collection('users').orderBy('createdAt','desc').get().then(function(snapshot) {
        ...
        snapshot.forEach(function(doc){
            ...
            var btn=u.authorized==='no'?'<button class="btn-add" style="padding:6px 10px;font-size:0.7rem;margin-right:5px;" onclick="authorizeUser(\''+id+'\')">Autoriser</button><button class="btn-delete" style="padding:6px 10px;font-size:0.7rem;" onclick="deleteUser(\''+id+'\')">Supprimer</button>':'<button class="btn-edit" style="padding:6px 10px;font-size:0.7rem;margin-right:5px;color:#d97706;" onclick="deauthorizeUser(\''+id+'\')">Bloquer</button><button class="btn-delete" style="padding:6px 10px;font-size:0.7rem;" onclick="deleteUser(\''+id+'\')">Supprimer</button>';
...
function authorizeUser(uid){if(confirm('Autoriser ?')){db.collection('users').doc(uid).update({authorized:'yes',updatedAt:firebase.firestore.FieldValue.serverTimestamp()}).then(function(){alert('OK');loadUsersList();}).catch(function(e){alert(e.message);});}}
function deauthorizeUser(uid){if(confirm('Bloquer ?')){db.collection('users').doc(uid).update({authorized:'no',updatedAt:firebase.firestore.FieldValue.serverTimestamp()}).then(function(){alert('OK');loadUsersList();}).catch(function(e){alert(e.message);});}}
function deleteUser(uid){if(confirm('Supprimer ?')){db.collection('users').doc(uid).delete().then(function(){alert('OK');loadUsersList();}).catch(function(e){alert(e.message);});}}
function refreshUsers(){loadUsersList();}

function loadClientOrders() {
    if(!currentUser)return;
    db.collection('orders').where('clientId','==',currentUser.uid).orderBy('createdAt','desc').get().then(function(snapshot) {
        var tbody=document.getElementById('clientOrdersTable'); if(!tbody)return;
        tbody.innerHTML='';
        if(snapshot.empty){tbody.innerHTML='<tr><td colspan="4">Aucune</td></tr>';return;}
        snapshot.forEach(function(doc){var o=doc.data();var d=o.createdAt?new Date(o.createdAt.seconds*1000).toLocaleDateString():'N/A';tbody.innerHTML+='<tr><td>#'+doc.id.slice(-6)+'</td><td>'+d+'</td><td>'+(o.total||0).toFixed(2)+' EUR</td><td>'+o.status+'</td></tr>';});
    }).catch(function(){});
}
...
function updateClientProfile(event) {
    ...
    if (newPassword.length > 0) {
        if (newPassword.length < 6) { msgEl.style.display='block'; msgEl.style.background='#fee2e2'; msgEl.style.color='#991b1b'; msgEl.textContent='Mot de passe: 6 caracteres minimum'; return false; }
        if (newPassword !== confirmPassword) { msgEl.style.display='block'; msgEl.style.background='#fee2e2'; msgEl.style.color='#991b1b'; msgEl.textContent='Mots de passe differents'; return false; }
    }
    ...
    db.collection('users').doc(currentUser.uid).update(updates).then(function() {
        ...
        msgEl.style.display='block'; msgEl.style.background='#dcfce7'; msgEl.style.color='#16a34a'; msgEl.textContent='Profil mis a jour !';
        currentUserData.telephone = newPhone;
        localStorage.setItem('currentUser', JSON.stringify(currentUserData));
    }).catch(function(error) {
        msgEl.style.display='block'; msgEl.style.background='#fee2e2'; msgEl.style.color='#991b1b';
        if (error.code === 'auth/requires-recent-login') { msgEl.textContent = 'Reconnectez-vous pour changer le mot de passe.'; }
        else { msgEl.textContent = 'Erreur: ' + error.message; }
    });
    return false;
}
