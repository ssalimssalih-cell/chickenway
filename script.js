    ...
    auth.onAuthStateChanged(function(user) {
        if (user) {
            ...
            db.collection('users').doc(user.uid).get().then(function(doc) {
                ...
                if (userData.authorized !== 'yes') { auth.signOut(); showAuthPage(); if(typeof showLoginError==='function'){setTimeout(function(){showLoginError('Compte en attente de validation.');},300);} return; }
                ...
                if (userData.role === 'client') { if(typeof showClientPage==='function') showClientPage(); }
