...
function initApp() {
    ...
    auth.onAuthStateChanged(function(user) {
        if (user) { currentUser = user; db.collection('users').doc(user.uid).get().then(function(doc) {
            if (!doc.exists) { auth.signOut(); showAuthPage(); return; }
            var ud = doc.data(); if (ud.authorized !== 'yes') { auth.signOut(); showAuthPage(); return; }
            window.currentUserData = { uid: doc.id, userData: ud }; localStorage.setItem('currentUser', JSON.stringify(window.currentUserData));
...
function buildMenu() {
    ...
    if (window.currentUserData && window.currentUserData.userData.role === 'admin') { items = [{p:'dashboard',i:'fa-th-large',l:'Dashboard'},{p:'pos',i:'fa-cash-register',l:'POS'},{p:'commandes',i:'fa-shopping-basket',l:'Commandes en ligne'},{p:'categories',i:'fa-layer-group',l:'Categories'},{p:'products',i:'fa-utensils',l:'Produits'},{p:'clients',i:'fa-users',l:'Clients'},{p:'fournisseurs',i:'fa-truck',l:'Fournisseurs'},{p:'ventes',i:'fa-shopping-cart',l:'Ventes'},{p:'credits',i:'fa-credit-card',l:'Credits'},{p:'depenses',i:'fa-money-bill-wave',l:'Depenses'},{p:'options',i:'fa-cog',l:'Options'}]; document.getElementById('sidebarRole').textContent = 'Admin'; }
    else if (window.currentUserData && window.currentUserData.userData.role === 'caissier') { items = [{p:'pos',i:'fa-cash-register',l:'POS'},{p:'commandes',i:'fa-shopping-basket',l:'Commandes en ligne'},{p:'ventes',i:'fa-shopping-cart',l:'Ventes'},{p:'credits',i:'fa-credit-card',l:'Credits'}]; document.getElementById('sidebarRole').textContent = 'Caissier'; }
