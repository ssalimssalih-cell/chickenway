...
function initApp() {
    ...
    // Cacher dashboard et client au début
...
function loadCategoriesPage(content) {
    content.innerHTML = '<div class="content-card">' +
        ...
        '<thead><tr><th>ID</th><th>Icone</th><th>Nom</th><th>Description</th><th>CA</th><th>Profit</th><th>Stock Total</th><th>Nb Produits</th><th>Date Creation</th><th>Actions</th></tr></thead>' +
...
function openCategoryForm(data) {
    ...
    var html = '<div class="form-row">' +
        '<div class="form-group"><label>Photo/Icone</label><input type="file" id="catImage" accept="image/*" onchange="previewImage(this, \'catImagePreview\')"><div id="catImagePreview">' + imagePreview + '</div></div>' +
        '<div class="form-group"><label>Icone FontAwesome</label><input type="text" id="catIcon" value="' + (data.icon || 'fa-folder') + '" placeholder="fa-folder"></div>' +
        '</div>' +
        '<div class="form-row">' +
        ...
        '</div>' +
        '<div class="form-row"><div class="form-group"><label>Stock Total</label><input type="number" id="catStock" value="' + (data.totalStock || 0) + '"></div></div>' +
...
async function saveCategory() {
    ...
    var imageFile = document.getElementById('catImage') ? document.getElementById('catImage').files[0] : null;
    var imageUrl = null;
    ...
    var data = {
        nom: nom,
        icon: document.getElementById('catIcon') ? document.getElementById('catIcon').value : 'fa-folder',
        description: document.getElementById('catDesc') ? document.getElementById('catDesc').value : '',
        ca: parseFloat(document.getElementById('catCA') ? document.getElementById('catCA').value : 0) || 0,
        profit: parseFloat(document.getElementById('catProfit') ? document.getElementById('catProfit').value : 0) || 0,
        totalStock: parseInt(document.getElementById('catStock') ? document.getElementById('catStock').value : 0) || 0,
...
function loadProductsPage(content) {
    content.innerHTML = '<div class="content-card">' +
        ...
        '<select id="categoryFilter" class="role-select" style="width:auto;" onchange="filterProducts()"><option value="">Toutes categories</option></select>' +
        '<button class="btn-add" onclick="openProductForm()"><i class="fas fa-plus"></i> Nouveau Produit</button>' +
        '</div></div>' +
        '<div class="table-container">' +
        '<table class="data-table">' +
        '<thead><tr><th>ID</th><th>Image</th><th>Nom</th><th>Categorie</th><th>Prix Achat</th><th>Prix Vente</th><th>Profit</th><th>Prix Promo</th><th>Stock</th><th>Vendues</th><th>CA</th><th>Temps Prep</th><th>Dispo</th><th>Date Creation</th><th>Actions</th></tr></thead>' +
...
async function loadCategoriesInFilter() {
    ...
    try {
        ...
        console.error('Erreur filtre categories:', e);
    }
}
...
async function openProductForm(data) {
    ...
    var html = '<div class="form-row">' +
        '<div class="form-group"><label>Image du produit</label><input type="file" id="prodImage" accept="image/*" onchange="previewImage(this, \'prodImagePreview\')"><div id="prodImagePreview">' + imagePreview + '</div></div>' +
        '</div>' +
        '<div class="form-row">' +
        '<div class="form-group"><label>Nom *</label><input type="text" id="prodNom" value="' + (data.nom || '') + '" required></div>' +
        '<div class="form-group"><label>Categorie</label><select id="prodCat"><option value="">Selectionner</option>' + categoriesOptions + '</select></div>' +
        '</div>' +
        '<div class="form-row">' +
        '<div class="form-group"><label>Prix Achat (MAD)</label><input type="number" id="prodPA" value="' + (data.prixAchat || 0) + '" step="0.01" onchange="calculateProfitPreview()"></div>' +
        '<div class="form-group"><label>Prix Vente (MAD)</label><input type="number" id="prodPV" value="' + (data.prixVente || 0) + '" step="0.01" onchange="calculateProfitPreview()"></div>' +
        '</div>' +
        '<div class="form-row">' +
        '<div class="form-group"><label>Prix Promo (MAD)</label><input type="number" id="prodPromo" value="' + (data.prixPromo || 0) + '" step="0.01" onchange="calculateProfitPreview()"></div>' +
        '<div class="form-group"><label>Profit: <span id="profitPreview" style="color:#16a34a;font-weight:700;">0.00 MAD</span></label></div>' +
        '</div>' +
        '<div class="form-row">' +
        '<div class="form-group"><label>Stock</label><input type="number" id="prodStock" value="' + (data.stock || 0) + '"></div>' +
        '<div class="form-group"><label>Temps Preparation</label><input type="text" id="prodTemps" value="' + (data.tempsPrep || '') + '" placeholder="ex: 15 min"></div>' +
        '</div>' +
        '<div class="form-row">' +
        '<div class="form-group"><label>Disponible</label><select id="prodDispo"><option value="1" ' + dispoYes + '>Oui</option><option value="0" ' + dispoNo + '>Non</option></select></div>' +
        '<div class="form-group"><label>Description</label><textarea id="prodDesc">' + (data.description || '') + '</textarea></div>' +
        '</div>' +
        '<button class="btn-cancel" onclick="closeModal()">Annuler</button>' +
...
function calculateProfitPreview() {
    var pa = parseFloat(document.getElementById('prodPA') ? document.getElementById('prodPA').value : 0) || 0;
    var pv = parseFloat(document.getElementById('prodPV') ? document.getElementById('prodPV').value : 0) || 0;
    var promo = parseFloat(document.getElementById('prodPromo') ? document.getElementById('prodPromo').value : 0) || 0;
...
async function saveProduct() {
    ...
    var imageFile = document.getElementById('prodImage') ? document.getElementById('prodImage').files[0] : null;
    var imageUrl = null;
    ...
    var prixAchat = parseFloat(document.getElementById('prodPA') ? document.getElementById('prodPA').value : 0) || 0;
    var prixVente = parseFloat(document.getElementById('prodPV') ? document.getElementById('prodPV').value : 0) || 0;
    var prixPromo = parseFloat(document.getElementById('prodPromo') ? document.getElementById('prodPromo').value : 0) || 0;
    ...
    var data = {
        nom: nom,
        categorie: document.getElementById('prodCat') ? document.getElementById('prodCat').value : '',
        prixAchat: prixAchat,
        prixVente: prixVente,
        prixPromo: prixPromo,
        profit: profit,
        ...
        vendues: 0,
        ca: 0,
        tempsPrep: document.getElementById('prodTemps') ? document.getElementById('prodTemps').value : '',
        disponible: document.getElementById('prodDispo') ? document.getElementById('prodDispo').value === '1' : true,
...
function loadClientsPage(content) {
    content.innerHTML = '<div class="content-card">' +
        '<div class="card-header"><h3><i class="fas fa-users"></i> Clients</h3><button class="btn-add" onclick="openClientForm()"><i class="fas fa-plus"></i> Ajouter Client</button></div>' +
        '<div class="table-container"><table class="data-table"><thead><tr><th>Nom</th><th>Prenom</th><th>Tel</th><th>CA</th><th>Points</th><th>Actions</th></tr></thead><tbody id="clientsTable"></tbody></table></div>' +
        '</div>';
    loadCollection('clients', 'clientsTable', function(id, d) {
        return '<tr><td><strong>' + d.nom + '</strong></td><td>' + d.prenom + '</td><td>' + (d.telephone || '-') + '</td><td>' + (d.ca || 0).toFixed(2) + ' MAD</td><td>' + (d.points || 0) + '</td><td><button class="btn-edit" onclick="editDocument(\'clients\',\'' + id + '\')"><i class="fas fa-edit"></i></button> <button class="btn-delete" onclick="deleteDocument(\'clients\',\'' + id + '\')"><i class="fas fa-trash"></i></button></td></tr>';
    });
}
...
function saveClient() {
    var nom = document.getElementById('cliNom') ? document.getElementById('cliNom').value : '';
    var prenom = document.getElementById('cliPrenom') ? document.getElementById('cliPrenom').value : '';
    if (!nom || !prenom) { alert('Nom et Prenom obligatoires'); return; }
    
    var data = {
        nom: nom,
        prenom: prenom,
        telephone: document.getElementById('cliTel') ? document.getElementById('cliTel').value : '',
        ca: parseFloat(document.getElementById('cliCA') ? document.getElementById('cliCA').value : 0) || 0,
        points: parseInt(document.getElementById('cliPoints') ? document.getElementById('cliPoints').value : 0) || 0,
        description: document.getElementById('cliDesc') ? document.getElementById('cliDesc').value : ''
    };
...
function loadFournisseursPage(content) {
    content.innerHTML = '<div class="content-card">' +
        '<div class="card-header"><h3><i class="fas fa-truck"></i> Fournisseurs</h3><button class="btn-add" onclick="openFournisseurForm()"><i class="fas fa-plus"></i> Ajouter Fournisseur</button></div>' +
        '<div class="table-container"><table class="data-table"><thead><tr><th>Nom</th><th>Prenom</th><th>Tel</th><th>Actions</th></tr></thead><tbody id="fournisseursTable"></tbody></table></div>' +
        '</div>';
...
function saveFournisseur() {
    var nom = document.getElementById('fourNom') ? document.getElementById('fourNom').value : '';
    var prenom = document.getElementById('fourPrenom') ? document.getElementById('fourPrenom').value : '';
    if (!nom || !prenom) { alert('Nom et Prenom obligatoires'); return; }
    
    var data = {
        nom: nom,
        prenom: prenom,
        telephone: document.getElementById('fourTel') ? document.getElementById('fourTel').value : '',
        description: document.getElementById('fourDesc') ? document.getElementById('fourDesc').value : ''
    };
...
function loadDepensesPage(content) {
    content.innerHTML = '<div class="content-card">' +
        '<div class="card-header"><h3><i class="fas fa-money-bill-wave"></i> Depenses</h3><button class="btn-add" onclick="openDepenseForm()"><i class="fas fa-plus"></i> Ajouter Depense</button></div>' +
        '<div class="table-container"><table class="data-table"><thead><tr><th>Date</th><th>Description</th><th>Montant</th><th>Actions</th></tr></thead><tbody id="depensesTable"></tbody></table></div>' +
        '</div>';
    loadCollection('depenses', 'depensesTable', function(id, d) {
        return '<tr><td>' + (d.date || 'N/A') + '</td><td>' + (d.description || '-') + '</td><td><strong style="color:#ef4444;">' + (d.montant || 0).toFixed(2) + ' MAD</strong></td><td><button class="btn-edit" onclick="editDocument(\'depenses\',\'' + id + '\')"><i class="fas fa-edit"></i></button> <button class="btn-delete" onclick="deleteDocument(\'depenses\',\'' + id + '\')"><i class="fas fa-trash"></i></button></td></tr>';
    });
}
...
function saveDepense() {
    var montant = parseFloat(document.getElementById('depMontant') ? document.getElementById('depMontant').value : 0) || 0;
    if (!montant) { alert('Montant obligatoire'); return; }
    
    var data = {
        date: document.getElementById('depDate') ? document.getElementById('depDate').value : '',
        montant: montant,
        description: document.getElementById('depDesc') ? document.getElementById('depDesc').value : ''
    };
...
function loadOptionsPage(content) {
    ...
    content.innerHTML = '<div class="stats-grid" style="margin-bottom:20px;">' +
        ...
        '</div>' +
        '<div class="content-card"><div class="card-header"><h3>Gestion utilisateurs</h3><button class="btn-add" onclick="loadUsersList()">Actualiser</button></div>' +
...
function handleRegister(event) {
    ...
    var nom = document.getElementById('regNom') ? document.getElementById('regNom').value.trim() : '';
    var prenom = document.getElementById('regPrenom') ? document.getElementById('regPrenom').value.trim() : '';
    ...
    var telephone = document.getElementById('regTelephone') ? document.getElementById('regTelephone').value.trim() : '';
