...
function buildMenu() {
    ...
    if (currentUserData.role === 'admin') {
        ...
        var items = [
            ...
            {page:'products',icon:'fa-utensils',label:'Produits'},
            {page:'clients',icon:'fa-users',label:'Clients'},
            {page:'fournisseurs',icon:'fa-truck',label:'Fournisseurs'},
            {page:'ventes',icon:'fa-shopping-cart',label:'Ventes'},
            {page:'credits',icon:'fa-credit-card',label:'Credits'},
            {page:'depenses',icon:'fa-money-bill-wave',label:'Depenses'},
            {page:'statistiques',icon:'fa-chart-bar',label:'Statistiques'},
            {page:'options',icon:'fa-cog',label:'Options'}
        ];
        items.forEach(function(item) {
            var li = document.createElement('li');
            li.className = 'nav-item';
            li.setAttribute('onclick', 'navigateTo(\''+item.page+'\')');
...
function loadCategoriesPage(content) {
    ...
    loadCollection('categories', 'categoriesTable', function(id, d) {
        ...
        return '<tr><td>'+(d.icon || '📁')+'</td><td><strong>'+d.nom+'</strong></td><td>'+(d.description || '-')+'</td><td>'+ca+'</td><td>'+profit+'</td><td>'+date+'</td><td><button class="btn-edit" onclick="editDocument(\'categories\',\''+id+'\')"><i class="fas fa-edit"></i></button> <button class="btn-delete" onclick="deleteDocument(\'categories\',\''+id+'\')"><i class="fas fa-trash"></i></button></td></tr>';
    });
}
...
function saveProduct() {
    var data = {
        nom: document.getElementById('prodNom').value,
...
function saveClient() {
    var data = {
        nom: document.getElementById('cliNom').value,
        prenom: document.getElementById('cliPrenom').value,
        telephone: document.getElementById('cliTel').value,
        whatsapp: document.getElementById('cliWA').value,
...
function saveDepense() {
    var data = {
        ...
        montant: parseFloat(document.getElementById('depMontant').value) || 0,
...
function loadUsersList() {
    db.collection('users').orderBy('createdAt','desc').get().then(function(snapshot) {
        ...
        snapshot.forEach(function(doc){
            ...
            var btn=u.authorized==='no'?'<button class="btn-add" style="padding:4px 8px;font-size:0.7rem;margin-right:5px;" onclick="authorizeUser(\''+id+'\')">Autoriser</button><button class="btn-delete" style="padding:4px 8px;font-size:0.7rem;" onclick="deleteUser(\''+id+'\')">Supprimer</button>':'<button class="btn-edit" style="padding:4px 8px;font-size:0.7rem;margin-right:5px;color:#d97706;" onclick="deauthorizeUser(\''+id+'\')">Bloquer</button><button class="btn-delete" style="padding:4px 8px;font-size:0.7rem;" onclick="deleteUser(\''+id+'\')">Supprimer</button>';
...
function clientNavigate(page) {
    ...
    document.getElementById('clientDynamicContent').innerHTML = '<div class="content-card"><h3>'+ (page === 'commander' ? 'Commander' : page === 'historique' ? 'Historique' : 'Parametres') +'</h3><p style="text-align:center;padding:40px;">Fonctionnalite a venir</p></div>';
}
