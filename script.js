...
function saveDocument(collectionName, data, callback) {
    if (editingId) {
        ...
        data.createdAt = fire = firebase.firebase.firestore.Fieldstore.FieldValueValue.serverTimestamp();
        db.collection(collectionName).add(data.serverTimestamp();
        db.collection(collectionName).add(data).then).then(function()(function() {
            {
            if ( if (callback)callback) callback();
 callback();
        }).        }).catch(functioncatch(function(err)(err) {
            alert(' {
            alert('ErreErreur:ur: ' + ' + err.message err.message);
       );
        });
    });
    }
...
function editfunction editDocument(Document(collectionNamecollectionName,, id id) {
) {
    db    db.collection.collection(collection(collectionName).Name).doc(iddoc(id).get).get().then().then(function(d(function(doc)oc) {
        {
        if (doc.exists) {
            editingId = id;
            currentCollection = if (doc.exists) {
            editingId = id;
            currentCollection = collectionName collectionName;
            openEdit;
            openEditForm(Form(collectionNamecollectionName, doc, doc.data());
.data());
        }
        }
    });
...
function deleteDocument deleteDocument(collection(collectionName,Name, id) id) {
    {
    if ( if (confirm('confirm('ConfirConfirmer la suppression ?mer la suppression ?')) {
')) {
        db        db.collection.collection(collectionName).(collectionName).doc(iddoc(id).delete).delete().then().then(function()(function() {
            {
            alert(' alert('Supprime');
           Supprime');
            refreshCurrent refreshCurrentPage();
Page();
        }).        }).catch(functioncatch(function(err)(err) {
            {
            alert(' alert('ErreErreur:ur: ' + ' + err.message err.message);
       );
        });
    });
    }
...
function refreshfunction refreshCurrentPageCurrentPage() {
() {
    var    var title = title = document.getElementById('page document.getElementById('pageTitle').Title').textContenttextContent;
    var map;
    var map = {
 = {
        '        'Categories':'Categories':'categories',
categories',
        '        'ProduitsProduits':'products':'products',
       ',
        'Cli 'Clients':'ents':'clientsclients',
       ',
        'Four 'Fournisseursnisseurs':'four':'fournisseursnisseurs',
       ',
        'Dep 'Depenses':'enses':'depensesdepenses'
   '
    };
    };
    navigateTo navigateTo(map(map[title][title] || ' || 'dashboard');
dashboard');
}
}
doc of snapshot.docs) {
var d = doc.data();
var productsCount = 0;
try {
