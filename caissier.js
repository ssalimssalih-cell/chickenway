...
function posResetCart() { posCart=[]; posStep=1; posSelectedCategory='all'; posCurrentClient=null; posCurrentTable=''; posPaymentMethod='espece'; posAmountGiven=0; posDiscountMAD=0; }

// ==================== RECHERCHE CLIENT DYNAMIQUE ====================
function posSearchClient(query) {
    var q = query.toLowerCase().trim();
    if (!q) { posFilteredClients = posAllClients.slice(); }
    else {
...
function posSelectClientFromDropdown(clientId) {
    if (!clientId) { posCurrentClient = null; return; }
    var cl = posAllClients.find(function(x) { return x.id === clientId; });
    if (cl) { posCurrentClient = {id: cl.id, name: cl.nom + ' ' + cl.prenom}; posCurrentTable = ''; }
    renderPOS();
}
...
function posSetTable(value) {
    ...
    if (posCurrentTable) {
        ...
        posPaymentMethod = 'espece'; // Table = paiement obligatoire
    }
    renderPOS();
}
...
function renderPOS() {
    ...
    if(posStep===1){
        ...
        var canCredit = posCurrentClient && posCurrentClient.id; // Crédit seulement si client sélectionné
        var canPartiel = posCurrentClient && posCurrentClient.id;
        ...
        h+='<div class="pos-payment-section"><label>Client</label><div style="position:relative;"><input type="text" id="posClientSearchInput" placeholder="🔍 Rechercher un client..." onkeyup="posSearchClient(this.value)" autocomplete="off" style="width:100%;padding:12px;border:2px solid #e2e8f0;border-radius:12px;font-size:0.9rem;"><div id="posClientDropdown" style="display:none;position:absolute;top:100%;left:0;right:0;background:#fff;border:2px solid #e2e8f0;border-radius:12px;max-height:200px;overflow-y:auto;z-index:50;box-shadow:0 10px 30px rgba(0,0,0,0.15);"></div></div></div>';
        ...
        h+='<button class="pos-payment-btn '+(posPaymentMethod==='credit'?'active':'')+'" onclick="posSetPaymentMethod(\'credit\')" '+(canCredit?'':'disabled style="opacity:0.4;cursor:not-allowed;" title="Crédit réservé aux clients"')+'><i class="fas fa-credit-card"></i> Crédit</button>';
        h+='<button class="pos-payment-btn '+(posPaymentMethod==='partiel'?'active':'')+'" onclick="posSetPaymentMethod(\'partiel\')" '+(canPartiel?'':'disabled style="opacity:0.4;cursor:not-allowed;" title="Partiel réservé aux clients"')+'><i class="fas fa-hand-holding-usd"></i> Partiel</button>';
        h+='</div></div>';
        ...
        setTimeout(function(){
            ...
            if(dropdown){
                ...
                posFilteredClients.forEach(function(cl){
                    ddHtml += '<div onclick="posSelectClientFromDropdown(\''+cl.id+'\')" style="padding:10px 12px;cursor:pointer;border-bottom:1px solid #f1f5f9;font-size:0.85rem;" onmouseover="this.style.background=\'#fef3c7\'" onmouseout="this.style.background=\'#fff\'">'+cl.nom+' '+cl.prenom+' <span style="color:#94a3b8;font-size:0.75rem;">('+(cl.telephone||'N/A')+')</span></div>';
                });
                dropdown.innerHTML = ddHtml;
                dropdown.style.display = posFilteredClients.length > 0 ? 'block' : 'none';
            }
        }, 100);
    }
...
function posSetPaymentMethod(m){
    if(m==='credit'||m==='partiel'){
        if(!posCurrentClient||!posCurrentClient.id){
            alert('Le crédit et le paiement partiel sont réservés aux clients enregistrés.\nVeuillez sélectionner un client.');
            return;
        }
    }
...
async function posFinalizeSale() {
    ...
    if(!posCurrentClient&&!posCurrentTable){alert('Veuillez sélectionner un client ou entrer un numéro de table.');return;}
    
    // Table = paiement obligatoire en espèces
    if(posCurrentTable&&(posPaymentMethod==='credit'||posPaymentMethod==='partiel')){
        alert('Pour une table, seul le paiement en espèces est autorisé.');
        return;
    }
    
    // Client obligatoire pour crédit/partiel
    ...
    try{
        ...
        if(posPaymentMethod==='credit'){
            ...
            posAmountGiven=parseFloat(document.getElementById('posAmountGiven').value)||0;
            remaining=t-posAmountGiven;paid=false;statutPaiement='partiel';change=Math.max(0,posAmountGiven-t);
        } else {
            posAmountGiven=parseFloat(document.getElementById('posAmountGiven').value)||0;
            change=posAmountGiven-t;paid=true;statutPaiement='payé';
        }
        
        // Si table et pas de client -> statut "en attente" jusqu'à paiement
        ...
        var itemsDetail=posCart.map(function(it){
            ...
            var profitArticle=(prixVenteReel-prixAchat)*it.quantite;
            profitTotal+=profitArticle;
            return {id:it.id,nom:it.nom,quantite:it.quantite,prixVente:prixVenteReel,prixAchat:prixAchat,prixPromo:prixPromo,profit:profitArticle,sauces:it.sauces||[],interdits:it.interdits||[],epice:it.epice||'Normal',sel:it.sel||'Normal'};
        });
        ...
        // Mise à jour CA et Profit client (seulement si payé)
        if(posCurrentClient&&posCurrentClient.id&&paid){
            try{var cr=await db.collection('clients').doc(posCurrentClient.id).get();if(cr.exists){var cd=cr.data();await db.collection('clients').doc(posCurrentClient.id).update({ca:(cd.ca||0)+t,profit:(cd.profit||0)+profitTotal,updatedAt:firebase.firestore.FieldValue.serverTimestamp()});}}catch(e){}
        }
