...
async function loadPosPage(c) {
    ...
    // Vérifier si une commande en ligne est à payer
...
function posResetCart() { posCart=[]; posStep=1; posSelectedCategory='all'; posCurrentClient=null; posCurrentTable=''; posPaymentMethod='espece'; posAmountGiven=0; posDiscountMAD=0; posFilteredClients = posAllClients.slice(); delete window.posCommandeId; delete window.posVenteId; }
...
async function posFinalizeSale(){
    ...
    try{
        ...
        // Mise à jour de la vente si paiement depuis liste ventes
        if(window.posVenteId){
            await db.collection('ventes').doc(window.posVenteId).update({paid:true,statutPaiement:'payé',remainingAmount:0,paidAt:firebase.firestore.FieldValue.serverTimestamp()});
            await db.collection('credits').where('factureNum','==',(await db.collection('ventes').doc(window.posVenteId).get()).data().factureNum).get().then(function(sn){sn.forEach(function(d){d.ref.update({paid:true,remainingAmount:0});});});
