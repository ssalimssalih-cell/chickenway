...
async function loadPosPage(c) {
    ...
    try {
        ...
        var cl = await db.collection('clients').orderBy('nom').get(); posAllClients = []; cl.forEach(function(d) { posAllClients.push({id:d.id,nom:d.data().nom,prenom:d.data().prenom,telephone:d.data().telephone}); });
...
function renderPOS() {
    var c=document.getElementById('dynamicContent');if(!c)return;var st=posCalculateTotal();var da=st*(posDiscount/100);var t=st-da;
    ...
    if(posStep===1){h+='<div class="pos-cart-header"><h3><i class="fas fa-shopping-cart"></i> Panier <span class="pos-cart-badge">'+posCart.length+'</span></h3><button class="pos-clear-btn" onclick="posResetCart()"><i class="fas fa-trash-alt"></i> Vider</button></div><div class="pos-cart-items">';
...
async function posFinalizeSale() {
    ...
    try{
        ...
        // Calcul du profit total de la vente
        var profitTotal=0;
        ...
        // MISE À JOUR CA ET PROFIT DU CLIENT
