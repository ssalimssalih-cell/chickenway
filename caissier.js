var posCart=[];var posStep=1;var posCategoriesList=[];var posProductsList=[];var posSelectedCategory='all';var posCurrentClient=null;var posCurrentTable='';var posPaymentMethod='espece';var posAmountGiven=0;var posDiscount=0;var posAllClients=[];var posCurrentProductId=null;
var posSaucesList=['Ketchup','Sauce Hot','Cheezy','Sauce Burger','Algérienne','Barbecue','Mayonnaise','Harissa','Samouraï','Andalouse','Biggy','Curry'];
var posInterditsList=['Oignon','Tomate','Cornichon','Olive','Fromage','Salade','Poivron','Champignon'];
var posEpicesList=['Normal','Moins épicé','Très épicé','Sans épice'];
var posSelList=['Normal','Moins de sel','Sans sel','Avec sel'];

async function loadPosPage(c){posResetCart();posStep=1;try{var cs=await db.collection('categories').get();posCategoriesList=[];cs.forEach(function(d){posCategoriesList.push({id:d.id,nom:d.data().nom,imageBase64:d.data().imageBase64});});var ps=await db.collection('products').get();posProductsList=[];ps.forEach(function(d){var dd=d.data();if(dd.disponible!==false)posProductsList.push({id:d.id,nom:dd.nom,prixVente:dd.prixVente||0,prixPromo:dd.prixPromo||0,prixAchat:dd.prixAchat||0,stock:dd.stock,categorie:dd.categorie||'',imageBase64:dd.imageBase64||''});});var cl=await db.collection('clients').orderBy('nom').get();posAllClients=[];cl.forEach(function(d){posAllClients.push({id:d.id,nom:d.data().nom,prenom:d.data().prenom,telephone:d.data().telephone,remise:d.data().remise||0});});}catch(e){}renderPOS();}
function posResetCart(){posCart=[];posStep=1;posSelectedCategory='all';posCurrentClient=null;posCurrentTable='';posPaymentMethod='espece';posAmountGiven=0;posDiscount=0;posCurrentProductId=null;}
...
function posOpenOptionsModal(pid){var p=posProductsList.find(function(x){return x.id===pid;});if(!p)return;if(p.stock!==undefined&&p.stock<=0){alert('Rupture');return;}posCurrentProductId=pid;
    ...
    h+='<div style="margin-bottom:15px;"><label style="font-weight:600;display:block;margin-bottom:5px;">🥫 Sauces:</label><div style="display:flex;flex-wrap:wrap;gap:6px;">';posSaucesList.forEach(function(s){h+='<label style="display:flex;align-items:center;gap:4px;padding:4px 8px;border:1px solid #e2e8f0;border-radius:6px;cursor:pointer;font-size:0.75rem;"><input type="checkbox" class="pos-sauce-check" value="'+s+'"> '+s+'</label>';});h+='</div></div>';
    h+='<div style="margin-bottom:15px;"><label style="font-weight:600;display:block;margin-bottom:5px;">🚫 Interdits:</label><div style="display:flex;flex-wrap:wrap;gap:6px;">';posInterditsList.forEach(function(s){h+='<label style="display:flex;align-items:center;gap:4px;padding:4px 8px;border:1px solid #e2e8f0;border-radius:6px;cursor:pointer;font-size:0.75rem;"><input type="checkbox" class="pos-interdit-check" value="'+s+'"> '+s+'</label>';});h+='</div></div>';
    h+='<div style="margin-bottom:15px;"><label style="font-weight:600;display:block;margin-bottom:5px;">🌶️ Épices:</label><div style="display:flex;flex-wrap:wrap;gap:6px;">';posEpicesList.forEach(function(s,idx){h+='<label style="display:flex;align-items:center;gap:4px;padding:4px 8px;border:1px solid #e2e8f0;border-radius:6px;cursor:pointer;font-size:0.75rem;"><input type="radio" name="pos-epice" class="pos-epice-radio" value="'+s+'" '+(idx===0?'checked':'')+'> '+s+'</label>';});h+='</div></div>';
    h+='<div style="margin-bottom:15px;"><label style="font-weight:600;display:block;margin-bottom:5px;">🧂 Sel:</label><div style="display:flex;flex-wrap:wrap;gap:6px;">';posSelList.forEach(function(s,idx){h+='<label style="display:flex;align-items:center;gap:4px;padding:4px 8px;border:1px solid #e2e8f0;border-radius:6px;cursor:pointer;font-size:0.75rem;"><input type="radio" name="pos-sel" class="pos-sel-radio" value="'+s+'" '+(idx===0?'checked':'')+'> '+s+'</label>';});h+='</div></div>';
...
function posConfirmOptions(){
    ...
    if(posCurrentClient&&posCurrentClient.id){saveClientPreferences(posCurrentClient.id,interdits);}
    closeModal();renderPOS();}
async function saveClientPreferences(cid,interdits){try{var cr=await db.collection('clients').doc(cid).get();if(cr.exists){var cd=cr.data();var ei=cd.interdits||[];interdits.forEach(function(i){if(ei.indexOf(i)===-1)ei.push(i);});await db.collection('clients').doc(cid).update({interdits:ei,updatedAt:firebase.firestore.FieldValue.serverTimestamp()});}}catch(e){}}
...
function renderPOS(){var c=document.getElementById('dynamicContent');if(!c)return;var st=posCalculateTotal();var da=st*(posDiscount/100);var t=st-da;
    ...
    if(posStep===1){h+='<div class="pos-cart-header"><h3><i class="fas fa-shopping-cart"></i> Panier <span class="pos-cart-badge">'+posCart.length+'</span></h3><button class="pos-clear-btn" onclick="posResetCart()"><i class="fas fa-trash-alt"></i></button></div><div class="pos-cart-items">';
    ...
    else if(posStep===2){h+='<div class="pos-cart-header"><h3><i class="fas fa-credit-card"></i> Paiement</h3><button class="pos-back-btn" onclick="posGoToStep1()"><i class="fas fa-arrow-left"></i> Retour</button></div><div class="pos-payment-form">';
        h+='<div class="pos-payment-section"><label>Client</label><select id="posClientSelect" onchange="posSelectClientFromList(this.value)" style="width:100%;padding:12px;border:2px solid #e2e8f0;border-radius:12px;"><option value="">— Aucun —</option>';
        for(var x=0;x<posAllClients.length;x++){var cl=posAllClients[x];h+='<option value="'+cl.id+'">'+cl.nom+' '+cl.prenom+(cl.telephone?' ('+cl.telephone+')':'')+' - Remise:'+(cl.remise||0)+'%</option>';}
    ...
    h+='</div></div>';c.innerHTML=h;if(posStep===2)setTimeout(posCalculateChange,200);}
...
function posSelectClientFromList(cid){if(!cid){posCurrentClient=null;return;}var cl=posAllClients.find(function(x){return x.id===cid;});if(cl){posCurrentClient={id:cl.id,name:cl.nom+' '+cl.prenom,remise:cl.remise||0};posCurrentTable='';document.getElementById('posTableNum').value='';posDiscount=cl.remise||0;posAmountGiven=0;renderPOS();}}
...
function posGoToStep2(){if(posCart.length===0){alert('Vide');return;}posStep=2;renderPOS();}
...
async function posFinalizeSale(){
    ...
    if(posPaymentMethod==='espece'){posAmountGiven=parseFloat(document.getElementById('posAmountGiven').value)||0;if(posAmountGiven<t){alert('Insuffisant');return;}}
    if(posPaymentMethod==='partiel'){posAmountGiven=parseFloat(document.getElementById('posAmountGiven').value)||0;if(posAmountGiven>=t)posPaymentMethod='espece';}
    ...
    try{
        ...
        var itemsDetail=posCart.map(function(it){return{id:it.id,nom:it.nom,quantite:it.quantite,prixVente:it.prixUnitaire,prixAchat:it.prixAchat||0,profit:(it.prixUnitaire-(it.prixAchat||0))*it.quantite,sauces:it.sauces||[],interdits:it.interdits||[],permis:[],epice:it.epice||'Normal',sel:it.sel||'Normal'};});
        ...
        if((posPaymentMethod==='espece'||posPaymentMethod==='partiel')&&posAmountGiven>t)msg+='\n💵 Rendu: '+change.toFixed(2)+' MAD';
        if(posPaymentMethod==='credit')msg+='\n📋 Credit';
        if(posPaymentMethod==='partiel')msg+='\n📋 Reste: '+remaining.toFixed(2)+' MAD';
