...
function handleLogin(event) {
    ...
    auth.signInWithEmailAndPassword(email, password)
        ...
        .catch(function(error) {
            ...
            else if (error.code === 'auth/network-request-failed') msg = 'Erreur réseau';
            showLoginError(msg);
        })
        .finally(function() {
            btn.disabled = false;
...
function handleRegister(event) {
    ...
    // Création d'un bandeau d'erreur si inexistant
    var regErrorEl = document.getElementById('registerError');
    if (!regErrorEl) {
        regErrorEl = document.createElement('div');
        regErrorEl.id = 'registerError';
        regErrorEl.style.cssText = 'background:#fee2e2;color:#991b1b;padding:12px 15px;border-radius:12px;margin-bottom:15px;font-size:0.9rem;text-align:center;border:2px solid #fecaca;display:none;';
        var regForm = document.getElementById('registerForm');
        if (regForm) regForm.parentNode.insertBefore(regErrorEl, regForm);
    }
    regErrorEl.style.display = 'none';

    if (!nom || !prenom || !username || !email || !telephone || !role || !password) {
        regErrorEl.innerHTML = '<i class="fas fa-exclamation-triangle"></i> Tous les champs sont obligatoires';
        regErrorEl.style.display = 'block';
        return false;
    }
    if (password.length < 6) {
        regErrorEl.innerHTML = '<i class="fas fa-exclamation-triangle"></i> Le mot de passe doit contenir au moins 6 caractères';
        regErrorEl.style.display = 'block';
        return false;
    }
    ...
    auth.createUserWithEmailAndPassword(email, password)
        ...
        .catch(function(error) {
            ...
            else if (error.code === 'auth/network-request-failed') msg = 'Erreur réseau. Vérifiez votre connexion internet.';
            ...
            regErrorEl.innerHTML = '<i class="fas fa-exclamation-triangle"></i> ' + msg;
            regErrorEl.style.display = 'block';
        })
        .finally(function() {
            btn.disabled = false;
