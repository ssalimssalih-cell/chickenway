...
function showLoginError(msg) {
    ...
    if (!el) { el = document.createElement('div'); el.id = 'loginError'; el.style.cssText = 'background:#fee2e2;color:#991b1b;padding:15px;border-radius:12px;margin-bottom:20px;font-size:0.9rem;text-align:center;border:2px solid #fecaca;'; var lf = document.getElementById('loginForm'); if(lf)lf.parentNode.insertBefore(el, lf); }
...
function handleRegister(event) {
    ...
    }).then(function() { alert('Compte cree ! En attente de validation.'); document.getElementById('registerForm').reset(); showLogin(); })
    .catch(function(e) { alert('Erreur: ' + e.message); })
    .finally(function() { btn.disabled = false; btn.innerHTML = '<i class="fas fa-user-plus"></i> Creer mon compte'; });
    return false;
}
